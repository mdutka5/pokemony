...
import { useFavoritePokemon } from '../context/FavoriteContext';
...
export default function PokemonDetailView({ pokemonName, isFavoriteScreen }: { pokemonName: string, isFavoriteScreen: boolean },
) {
  ...
  const displayImage = details.sprites.other?.['official-artwork']?.front_default || details.sprites.front_default;

  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.card}>
        <Text style={styles.pokeId}>#{details.id.toString().padStart(3, '0')}</Text>
      ...
      <View style={styles.statsContainer}>
        {favoritePokemon && details.name.toLowerCase() === favoritePokemon.toLowerCase() ? (
          <Pressable onPress={() => setFavoritePokemon(null)}>
            {isFavoriteScreen ? (
              <Text style={styles.alreadyFavoriteButton}>Remove from Favorites</Text>
            ) : (
...
const styles = StyleSheet.create({
    ...
    pokeName: { fontSize: 26, fontWeight: '900', color: '#212529', letterSpacing: 0.5 },
    image: { width: 180, height: 180, marginVertical: 10 },
    badgeRow: { flexDirection: 'row', gap: 8, marginTop: 10 },
    badge: { backgroundColor: '#e9ecef', paddingVertical: 6, paddingHorizontal: 16, borderRadius: 20 },
    badgeText: { fontSize: 14, fontWeight: '700', color: '#495057', textTransform: 'capitalize' },
    statsContainer: {
      backgroundColor: '#fff',
      borderRadius: 20,
      padding: 20,
    ...
    sectionTitle: { fontSize: 18, fontWeight: '800', color: '#212529', marginBottom: 15 },
    grid: { flexDirection: 'row', gap: 15 },
    gridItem: { flex: 1, backgroundColor: '#f8f9fa', borderRadius: 12, padding: 15, alignItems: 'center' },
    gridLabel: { fontSize: 11, fontWeight: '700', color: '#adb5bd', marginBottom: 4 },
